...
public class ExportTaxReportProcessTest {
    ...
    @Test
    public void testFormatCSVLine_columnOrder() {
        String line = ExportTaxReportProcess.formatCSVLine(
            2026, 2,
        ...
        assertEquals("year",                  "2026",   fields[0]);
        assertEquals("period",                "2",      fields[1]);
        assertEquals("TaxableRevenue",        "500000", fields[2]);
        assertEquals("ZeroRateSalesAmount",   "100000", fields[3]);
        assertEquals("ExemptRevenue",         "50000",  fields[4]);
        assertEquals("OutputTaxAmount",       "20000",  fields[5]);
        assertEquals("InputTaxAmount",        "10000",  fields[6]);
        assertEquals("NonDeductibleInputTax", "1000",   fields[7]);
        assertEquals("CarryOverTaxCredit",    "2000",   fields[8]);
        assertEquals("TaxPayable",            "7000",   fields[9]);
    }
